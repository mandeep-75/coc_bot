...
def click_button(image_utils, adb, image_folder, button_name, confidence_threshold=0.6, attempts=3):
    """Click a button with a specified number of attempts."""
    for attempt in range(attempts):
        logging.info(f"Attempting to click '{button_name}' (attempt {attempt + 1}/{attempts})")
        if image_utils.find_and_click_image(adb, image_folder, button_name, confidence_threshold):
            logging.info(f"✅ Successfully clicked '{button_name}'")
            return True
        else:
            logging.warning(f"⚠️ Could not find '{button_name}' on attempt {attempt + 1}")
